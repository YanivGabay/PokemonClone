...
{
public:
	GuideState(Stack<BaseState>& states, sf::Vector2f cameraCenter)
		: BaseState(states),
		  m_cameraCenter(cameraCenter),
		  m_prompt(std::move(std::make_unique<Gui>(Resources::getInstance().getFont(), sf::Vector2f(SCREEN_WIDTH,SCREEN_HEIGHT), sf::Vector2f(m_cameraCenter.x - SCREEN_WIDTH / 2, m_cameraCenter.y)))),
