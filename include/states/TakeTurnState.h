...
{
public:
	TakeTurnState(Stack<BaseState>& states,
				  std::shared_ptr<Battle> battle,
				  std::shared_ptr<Pokemon> playerPokemon,
				  std::shared_ptr<Pokemon> enemyPokemon,
				  WhosAttack ID)
		: BaseState(states),
		  m_battle(battle),
		  m_playerPokemon(playerPokemon),
		  m_wildPokemon(enemyPokemon),
		  m_id(ID),
		  m_attackingPokemon(ID == WhosAttack::Player ? playerPokemon->getBackSprite() : enemyPokemon->getFrontSprite()),
	...
	virtual ~TakeTurnState() = default;
	
	void entry()override
	{
		m_id == WhosAttack::Player ? m_battle->setAdviceText("Attack!!") :
									 m_battle->setAdviceText("Be Careful!");
	}
	...
	void exit() override
	{
		...
		{
			 state = std::make_unique<TakeDamageState>(getStateStack().get(), m_battle,m_wildPokemon,m_playerPokemon, m_id);
		}
		else
		{
			 state = std::make_unique<TakeDamageState>(getStateStack().get(), m_battle, m_playerPokemon, m_wildPokemon, m_id);
		}
