...
{
public:
	AddExpState(Stack<BaseState>& states, std::shared_ptr<Player> player, std::shared_ptr<Pokemon> pokemon, std::shared_ptr<Battle> battle)
		: m_player(player),
		  m_wildPokemon(pokemon),
		  m_battle(battle),
		  m_expProgress(std::move(std::make_unique<ProgressBar>(400, 452.0f, 300, 30, sf::Color::Black, sf::Color::Blue))),
