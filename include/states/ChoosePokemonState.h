...
{ 
public:
	ChoosePokemonState(Stack<BaseState>& states,sf::Vector2f cameraCenter,std::shared_ptr<Player> player)
		: BaseState(states),
		  ...
		  m_prompt(std::move(std::make_unique<Gui>(Resources::getInstance().getFont(),sf::Vector2f(m_windowSize.x, m_windowSize.y), sf::Vector2f(m_cameraCenter.x-m_windowSize.x/2,m_cameraCenter.y))))
	{
		int counter = 0;
